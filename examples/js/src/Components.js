...
export const createInfoComponent = () => {
	...
	language.innerHTML = 'JS WorkerUrl Example';
	...
	srcLink.href = 'https://github.com/popelenkow/worker-url/tree/main/examples/js/';
	result.appendChild(srcLink);

	return result;
};

export const createWorkerComponent = (messenger) => {
	const result = document.createElement('div');
	result.style.border = 'solid';
	result.style.padding = '5px';
	result.style.display = 'flex';
...
export const createWorkletComponent = (oscillator) => {
	const result = document.createElement('div');
	result.style.border = 'solid';
	result.style.padding = '5px';
	result.style.display = 'flex';
